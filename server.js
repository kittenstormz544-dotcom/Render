...
const BUCKET_SCENES = 'scene-videos'; 
...
function ensureFullUrl(input, bucket) {
    ...
    // FIX: Detect and fix double URLs (e.g. https://...https://...)
...
async function downloadAsset(url, scriptId, assetName) {
    if (!url) return null;
    console.log(`[DOWNLOAD] Attempting ${assetName}: ${url}`);
    try {
...
async function processJob(scriptId) {
    try {
        const { data: job, error: fetchErr } = await supabase.from('story_script').select('*').eq('id', scriptId).single();
        ...
        if (sd.scenes && Array.isArray(sd.scenes)) {
            for (let i = 0; i < sd.scenes.length; i++) {
                ...
                if (scene.video_url) {
                    const sPath = await downloadAsset(ensureFullUrl(scene.video_url, BUCKET_SCENES), scriptId, `scene_${i}`);
        ...
        // Fallback: If no scenes are found yet, create a 5 second placeholder so it doesn't crash
        if (videoPaths.length === 0) {
            console.log("[WARNING] No video clips found. Creating placeholder.");
            const placeholder = path.join('/tmp', `placeholder_${scriptId}.mp4`);
            await new Promise((res) => {
                const p = spawn('ffmpeg', ['-f', 'lavfi', '-i', `color=c=black:s=1280x720:d=5`, '-vf', `drawtext=text='Waiting for Scenes':fontcolor=white:fontsize=40:x=(w-text_w)/2:y=(h-text_h)/2`, '-pix_fmt', 'yuv420p', '-y', placeholder]);
                p.on('close', res);
            });
        ...
        console.error(`[ERROR] ID ${scriptId}:`, e.message);
